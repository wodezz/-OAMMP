...
class SvnTools(object):
    ...
    def log(self, path, number, user, passwd):
        vList = []
        cmd = "cd {path} && svn --username {user} --password {passwd} log -l {number} -q".format(path=path, number=number, user=user, passwd=passwd)
    ...
    def checkOut(self, path, name=None, user=None, passwd=None):
        cmd = "cd {path} && svn update --username {user} --password {passwd}".format(path=path, user=user, passwd=passwd)
        return commands.getstatusoutput(cmd)
    
    def clone(self, url, dir, user=None, passwd=None):
        if user and passwd:
            cmd = "svn co {url}  --username {user} --password {passwd} {dir}".format(url=url,user=user,passwd=passwd,dir=dir)
        else:
            cmd = "svn co {url}  {dir}".format(url=url,dir=dir)
        return commands.getstatusoutput(cmd)
    
    def pull(self, path, user, passwd):
        cmd = "cd {path} && svn update --username {user} --password {passwd}".format(path=path, user=user, passwd=passwd)
        return commands.getstatusoutput(cmd)
