...
from OpsManage.models import Server_Assets, Project_Config, Project_Number, Project_Order, Log_Project_Config
...
def deploy_add(request):
    ...
    if request.method == "GET":
        ...
        return render(request, 'deploy/deploy_add.html', locals())
...
def deploy_modf(request, pid):
    try:
        ...
        return render(request, 'deploy/deploy_modf.html', locals())
    if request.method == "GET": 
        groupList = Group.objects.all()
        servers = [ s.server for s in tagret_server]
...
def deploy_list(request):
    ...
    return render(request, 'deploy/deploy_list.html', locals())
...
def deploy_run(request, pid):
    try:
        project = Project_Config.objects.get(id=pid)  # 取出对应id的项目
        serverList = Project_Number.objects.filter(project=project)  # 取出对应项目的目标机器ip列表
        if project.project_repertory == 'git':
            version = GitTools()
        elif project.project_repertory == 'svn':
            version = SvnTools()  # 实例化SvnTools
    except:
        errorInfo = "项目不存在，可能已经被删除."
        return render(request, 'deploy/deploy_run.html', locals())
    if request.method == "GET":
        if project.project_model == 'branch':
            bList = version.branch(path=project.project_repo_dir)
        elif project.project_model == 'tag':
            bList = version.tag(path=project.project_repo_dir) # 获取了一个空列表?
        #获取最新版本
        result = version.pull(path=project.project_repo_dir, user=project.project_repo_user, passwd=project.project_repo_passwd)
        vList = version.log(path=project.project_repo_dir, number=50, user=project.project_repo_user, passwd=project.project_repo_passwd)
        return render(request, 'deploy/deploy_run.html', locals())
    elif request.method == "POST":
        print request.POST
        print request.POST.getlist('project_server')
        if request.POST.getlist('project_server',None):
            ...
            tmpServer = [i for i in allServerList if i not in request.POST.getlist('project_server')]  # wei kong
        else:
        ...
        if DsRedis.OpsProject.get(redisKey=project.project_uuid+"-locked") is None:#判断该项目是否有人在部署
            ...
            if request.POST.get('project_mode',None) == "rollback":
                ...
                if project.project_model == 'branch':
                    ...
                    DsRedis.OpsDeploy.lpush(project.project_uuid, data="[Start] Switchrred to branch %s" % bName)
                    #reset到指定版本
                    ...
                    bName = request.POST.get('project_branch')
                    print bName
                    print project.project_repo_user
                    print project.project_repo_passwd
                    print project.project_repo_dir
                    result = version.checkOut(path=project.project_repo_dir, name=bName, user=project.project_repo_user, passwd=project.project_repo_passwd)
            ...
            if resource and hostList:
                if exclude:
                    args = '''src={srcDir} dest={desDir} links=yes recursive=yes compress=yes delete=yes rsync_opts="{exclude}"'''.format(srcDir=softdir, desDir=ds.dir,exclude=exclude)
                else:
                    args = '''src={srcDir} dest={desDir} links=yes recursive=yes compress=yes delete=yes'''.format(srcDir=softdir, desDir=ds.dir)
...
def deploy_ask(request, pid):
    try:
        ...
        errorInfo = "项目不存在，可能已经被删除."
        return render(request, 'deploy/deploy_ask.html', locals())
    if request.method == "GET":
        vList = None
        version.pull(path=project.project_repo_dir, user=project.project_repo_user, passwd=project.project_repo_passwd)
        if project.project_model == 'branch':
            #获取最新版本
            bList = version.branch(path=project.project_repo_dir) 
...
def deploy_order(request):
    if request.method == "GET":
        ...
        return render(request, 'deploy/deploy_order.html', locals())
...
def deploy_manage(request, pid):
    try:
        ...
        errorInfo = "项目不存在，可能已经被删除."
        return render(request, 'deploy/deploy_manage.html', locals())
    if request.method == "GET":
        #获取最新版本
        version.pull(path=project.project_repo_dir, user=project.project_repo_user, passwd=project.project_repo_passwd)
        if project.project_model == 'branch':
